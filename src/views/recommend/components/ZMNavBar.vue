<template>
  <div class="scroll-box" ref="scrollBox">
    <div ref="scrollItem" class="item" :class="{ active: isSelected == name }" v-for="(item, name, index) in tabListData" :key="name" @click.stop="start(name, index)">
      {{ item }}
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    tabListData(n, o) {
      if (n) {
        const _index = this.tabListData && Object.keys(this.tabListData).indexOf(this.acticeIndex + '');
